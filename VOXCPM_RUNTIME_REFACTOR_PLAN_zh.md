...
- `2026-04-20`: 最终 `AudioVAE` chunk decode 已把“prepended prompt context”与“chunk history overlap”解耦；现在即使没有 prompt 音频，也会为每个后续 chunk 保留固定的左侧 latent 历史，避免 VoxCPM2 长文本在 CUDA fallback 下按 64 帧硬切独立解码后出现明显时序/语序错乱。
- `2026-04-20`: CLI 长序列最终解码已与 service 对齐，默认不再优先走 long-seq `output_pool` 最终导出；同时 `AudioVAE` chunk history 默认提高到 `64 / 256`（普通 / conditioned/deeper decoder），并支持通过 `VOXCPM_AUDIO_DECODE_HISTORY_FRAMES` 覆盖，便于继续钉 VoxCPM2 长文本 chunk 边界吞词问题。
- `2026-04-20`: `AudioVAE` chunk decode 现在会同时约束 history 与单窗口 latent patch 总数：conditioned/deeper decoder 默认 history 收敛到 `192`，并按安全窗口上限自动收缩 chunk size；CLI / service 也支持 `VOXCPM_AUDIO_DECODE_HISTORY_FRAMES` 与 `VOXCPM_AUDIO_DECODE_CHUNK_FRAMES` 覆盖，避免“为补上下文而再次撞回 CUDA transpose grid 上限”。
- `2026-04-20`: `AudioVAE` 的编码对齐长度与解码输出长度已显式拆开：prompt/reference 音频对齐继续使用 `encoder hop_length`，但流式/最终 waveform decode 的 chunk 裁剪、history trim、prompt-context trim 全部改为基于 `decoder_rates` 的 `decode_hop_length`；这修复了 VoxCPM2 这类非对称 AudioVAE 在长文本 chunk decode 时因样本步长算错导致的吞词/错位。
